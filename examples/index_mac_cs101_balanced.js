...
const { IEC101MasterBalanced } = require('./builds/macos_arm64/addon_iec60870');
const util = require('util');

// CS101 Master: Connects to a device via serial port in balanced mode
const master = new IEC101MasterBalanced((event, data) => {
    try {
        ...
        if (event === 'data' && Array.isArray(data)) {
            ...
            data.forEach(item => {
                console.log(`  IOA: ${item.ioa}, Value: ${item.val}, Quality: ${item.quality}${item.timestamp ? `, Timestamp: ${item.timestamp}` : ''}`);
            });
            console.log('Master: Sending control commands...');
            master.sendCommands([
            ...
            // In balanced mode, activation is automatic, but we explicitly call sendStartDT to ensure data flow starts
            master.sendStartDT();
